@testset "SimpleGrid" begin
    ...
    grid = Grid2D((-2, 2), 50)
    generator = Amoebas.startvalue_generator(F)
    B = Amoebas.simple_grid(F, grid, generator)
    @test any(B.data)
    B = Amoebas.amoeba(f, grid=grid, alg=Simple())
    @test any(B.data)

    I = Amoebas.imaginary_projection(f, grid=grid, alg=Simple())
    @test any(I.data)
    ...
    generator = Amoebas.DomainStartValueGenerator2D((-5, 5), (-5, 5))
